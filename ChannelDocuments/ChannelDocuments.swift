...
class ChannelDocuments: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBOutlet weak var btnadddocuments: UIButton!
    @IBAction func btnadddocuments(_ sender: Any) {
    }
    ...
    override func viewDidLoad() {
        ...
        btnadddocuments.layer.cornerRadius = btnadddocuments.frame.width / 2
        if NEWsChannelisAdmin == true{
            btnadddocuments.isHidden = false
        }
        else{
            btnadddocuments.isHidden = true
        }
...
//            userid = ""
//        }
//        let url = DataContainer.baseUrl()+"news/\(NEWsChannelID)/channel?channelid=\(NEWsChannelID)&userId=\(userid)&type=All&subType=\("document")&hashtag=All&limit=\(40)&offset=\(0)"
//
//        print(url)
...
//    }
    ...
    func GetAllChannelDocuments()
    {
        ...
        let url = BASEURL + "news/\(NEWsChannelID)/channel?channelid=\(NEWsChannelID)&userId=\(userid)&type=All&subType=\("document")&hashtag=\(NEWsChannelSEARCHTags)&limit=\(40)&offset=\(0)&eventDate=\(NEWsChannelEventDate)&FromDate=\(NEWsChannelFromDate)&ToDate=\(NEWsChannelToDate)&search=\(NEWsChannelSEARCHTitle)"
    ...
    // Get Location Name from Latitude and Longitude
        func GetMoreLinkPosts()
        {
          ...
          //  let url = DataContainer.baseUrl()+"news/\(channelID)/channel?userid=\(userid)&offset=\(lastid)"
            let url = BASEURL + "news/\(NEWsChannelID)/channel?channelid=\(NEWsChannelID)&userId=\(userid)&type=All&subType=\("document")&hashtag=\(NEWsChannelSEARCHTags)&limit=\(40)&offset=\(lastid)&eventDate=\(NEWsChannelEventDate)&FromDate=\(NEWsChannelFromDate)&ToDate=\(NEWsChannelToDate)&search=\(NEWsChannelSEARCHTitle)"
...
extension ChannelDocuments: CZPickerViewDelegate, CZPickerViewDataSource {
...
extension ChannelDocuments: IndicatorInfoProvider {
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        let itemInfo = IndicatorInfo(title: "DOCUMENTS")
        return itemInfo
    }
