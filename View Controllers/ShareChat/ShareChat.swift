            ...
            self.uploadMedia(type: selectedMessageType) { url in
                ...
                for (countIndex, index) in self.arrSelectedNumber.enumerated(){
                    if self.arrMsgGroupType[index as! Int] as! String == PUBLICGROUP
                    {
                        ...
                        if countIndex as! Int == self.arrSelectedNumber.count - 1{
                    ...
                    if snapshot.childrenCount == 0{
                        ...
                        if let snapShot = snapshot.children.allObjects as? [DataSnapshot] {
                            ...
                            if snapShot.count > 0
                            {
                                ...
                                if tempArrGroupParticipant.contains(defaults.value(forKey: "uid") as! String){
...
//            })
    ...
    func uploadMedia(type: Int, completion: @escaping (_ url: String?) -> Void) {
        ...
        if uploadData.isEmpty !=  true{
            ...
            if MEDIAPROGRESS == Float(){
                obj.showProgressBar(viewController: self)
                DispatchQueue.main.async {
            ...
            for (index, tempUid) in self.arrGroupParticipatUid.enumerated(){
                ...
                UserDB.child(tempUid as! String).observeSingleEvent(of: .value, with: { (snapshot) in
                    ...
                    if snapshot.childrenCount > 0
                    {
                        let datadic = (snapshot.value as! NSDictionary)
                        let allkeys = datadic.allKeys as NSArray
                        if self.arrGroupParticipatUserPhone.contains(datadic.value(forKey: "\(objUserDBM.phoneNumber)") as Any){
                            return
                        }
                        else{
                            self.arrGroupParticipatUserPic.add(datadic.value(forKey: "\(objUserDBM.profilePhoto)") as Any)
                            self.arrGroupParticipatUserName.add(datadic.value(forKey: "\(objUserDBM.userName)") as Any)
                            self.arrGroupParticipatUserPhone.add(datadic.value(forKey: "\(objUserDBM.phoneNumber)") as Any)
                            self.arrGroupParticipatUserFBToken.add(datadic.value(forKey: "\(objUserDBM.fcmId)") as Any)
                            self.arrGroupParticipatUserFBid.add(snapshot.key)
                            
                            if allkeys.contains("\(objUserDBM.source)"){
                                self.arrGroupParticipatDeviceSource.add((datadic.value(forKey: "\(objUserDBM.source)") as Any))
                            }
    ...
    func funSendMsgInGroup(tempMsgType: Int, messageTextt: String, tempfromuid: String, temptouid: String, tempgroupType: String, tempgroupId: String, isBack: Bool, tempreceiverId: String, receiverToken: String, isAndroidUserTemp: Bool, tempautocreatedid: String) {
        ...
        case (TEXT):
            dicmsg = [
                "\(objMessageDBM.from)" :tempfromuid,
                "\(objMessageDBM.message)" :messageTextt,
                "\(objMessageDBM.messageStatus)" :NOT_DELIVERED,
       ...
       case DOCUMENT:
            ...
            dicmsg = [
            "\(objMessageDBM.from)" :tempfromuid,
            "\(objMessageDBM.message)" :self.selectedFileName,
            "\(objMessageDBM.messageImagePath)" :self.objecturl,
            "\(objMessageDBM.messageStatus)" :NOT_DELIVERED,
        ...
        .setValue(dicmsg, withCompletionBlock: {
            ...
            else
            {
                ...
                if tempreceiverId != USERUID {
                    obj.SendPushNotification(toToken: receiverToken, title: "1 new \(tempMsgType) message", body: messageTextt, data: parameters )
                }
