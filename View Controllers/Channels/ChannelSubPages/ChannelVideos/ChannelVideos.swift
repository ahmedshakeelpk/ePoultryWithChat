...
class ChannelVideos: UIViewController, UITableViewDataSource, UITableViewDelegate, AVPlayerViewControllerDelegate {
     var arrname = [String]()
          var arrdesc = [String]()
...
//        let vc = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "Video_image") as! Video_image
//        vc.videoimagetag = "video"
//        vc.videoimagename = arrvideos[tag] as? String ?? ""
//        vc.strurl = videopath + "\(arrvideos[tag])"
//        self.navigationController?.pushViewController(vc, animated: true)
       
        let strurl = videopath + "\(arrvideourl[tag])" + videoPlayEndPath
    ...
    func GetAllChannelVideos()
    {
        ...
        let url = BASEURL + "news/\(NEWsChannelID)/channel?channelid=\(NEWsChannelID)&userId=\(userid)&type=All&subType=\("video")&hashtag=\(NEWsChannelSEARCHTags)&limit=\(40)&offset=\(0)&eventDate=\(NEWsChannelEventDate)&FromDate=\(NEWsChannelFromDate)&ToDate=\(NEWsChannelToDate)&search=\(NEWsChannelSEARCHTitle)"
    ...
    func GetMoreVideoPosts()
    {
      ...
      //  let url = DataContainer.baseUrl()+"news/\(channelID)/channel?userid=\(userid)&offset=\(lastid)"
        ...
        let url = BASEURL + "news/\(NEWsChannelID)/channel?channelid=\(NEWsChannelID)&userId=\(userid)&type=All&subType=\("video")&hashtag=\(NEWsChannelSEARCHTags)&limit=\(40)&offset=\(lastid)&eventDate=\(NEWsChannelEventDate)&FromDate=\(NEWsChannelFromDate)&ToDate=\(NEWsChannelToDate)&search=\(NEWsChannelSEARCHTitle)"
...
// Marks : -  Custom CZPicker View
 extension ChannelVideos: CZPickerViewDelegate, CZPickerViewDataSource {
...
extension ChannelVideos: IndicatorInfoProvider {
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        let itemInfo = IndicatorInfo(title: "VIDEOS")
        return itemInfo
    }
