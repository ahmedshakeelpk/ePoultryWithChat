...
class Comments: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, EmojiViewDelegate, UITextViewDelegate {
   ...
   ////////////////////////////////////////////////////////
    override func viewDidLoad() {
        self.tablev.register(UINib(nibName: "CommentsCell", bundle: nil), forCellReuseIdentifier: "cell")
        getallcomments()
        if isNotification {
            //MARK:- Cell Register
        ...
        txtcomment.layer.cornerRadius = txtcomment.frame.height / 2
        
        btnlike.setTitle("\(totallike+" liked this")", for: .normal)
    ...
    @objc func txtcommentfun() {
        self.tablev.frame = CGRect(x: 0, y: 40, width: self.tablev.frame.size.width, height: self.view.frame.size.height - 93)
        self.view.frame.origin.y = 0
        self.view.frame.origin.y = -180
        self.tablev.frame = CGRect(x: 0, y: 240, width: self.tablev.frame.size.width, height: self.tablev.frame.size.height - 173)
   }
    ...
    // Marks: - smiles tag button
    @IBAction func btntag(_ sender: Any) {
        let emojiView = EmojiView()
        emojiView.delegate = self
        txtcomment.inputView = emojiView
        txtcomment.becomeFirstResponder()
    }
    // Marks: - comments send button
    @IBAction func btnsend(_ sender: Any) {
        if txtcomment.text == "" {
            obj.showAlert(title: "Alert!", message: "Please type comments then send.", viewController: self)
        }
        else {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        self.view.endEditing(true)
        self.view.frame.origin.y = 64
        self.tablev.frame = CGRect(x: 0, y: 0, width: self.tablev.frame.size.width, height: self.tablev.frame.size.height + 180)
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        self.view.endEditing(true)
        self.view.frame.origin.y = 64
        self.tablev.frame = CGRect(x: 0, y: 40, width: self.tablev.frame.size.width, height: self.view.frame.size.height - 53)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if isNotification && indexPath.section == 0 && postType != "" {
            ...
            let cell = self.tablev.dequeueReusableCell(withIdentifier: "cell") as! CommentsCell
            cell.lblusername.text = arrusername[indexPath.row]
            cell.lbltime.text = arrtime[indexPath.row]
            cell.lblcomment.text = arrcomments[indexPath.row]
            //Kingfisher Image upload
            let urlprofile = URL(string: userimagepath+arrprofileimage[indexPath.row])
        ...
        obj.webServicesGetwithJsonResponse(url: url, completionHandler: {
                ...
                DispatchQueue.main.async {
                    if error == nil && responseObject != nil {
                        if let success = responseObject?.value(forKey: "success") as? String{
                            if success != "true" {
                        ...
                        if responseObject!.count > 0 {
                            let tempdatadic = (responseObject! as NSDictionary)
                                    ...
                                    if datadic.count > 0 {
                                        if datadic.count > 0 {
                                            for temp in datadic {
                                                ...
                                                //MARK:- Pehlay yahan username ta change kar k Name kar diya ha
                                                ...
                                                if let Comment = dict.value(forKey: "Comment") as? String {
                                                    self.arrcomments.append(Comment)
                                                }
                                                else {
                                                    self.arrcomments.append("")
                                                }
                                                if let profile_img = dict.value(forKey: "profile_img") as? String {
                                                    self.arrprofileimage.append(profile_img)
                                                }
                                                else {
                                                    self.arrprofileimage.append("")
                                                }
                                                if let CommentID = dict.value(forKey: "CommentID") as? String {
                                                    self.arrcommentid.append(CommentID)
                                                }
                                                else {
                                                    self.arrcommentid.append("")
                                                }
                                                
                                                if let CreatedOn = dict.value(forKey: "Creationdate") as? String {
...
//    try {
//        body.put("userId", String.valueOf(prefs.getUserId()));
//        body.put("comment", Utility.getSmileByUnicode(commentET.getText().toString().replace("'", "%dd")));
//        body.put("isNew", "1");
//        body.put("groupId", String.valueOf(0));
//        Utility.logcat("comment json object " + body.toString());
//    } catch (JSONException e) {
//        e.printStackTrace();
//    }
//
//    String url = Utility.WEB_API_PATH + "comment/" + statusId;
//
    func sendcomment() {
        let str = txtcomment.text!
        //str = String(describing: str.cString(using: String.Encoding.utf8))

        let url = BASEURL + "comment/\(statusid)"
        ...
        obj.webService(url: url, parameters: parameters, completionHandler: { responseObject, error in
            ...
            if error == nil && responseObject != nil {
                if (responseObject!.value(forKey: "data") as? String) != nil {
                    if self.type == "Groups" {
                        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "updategroupcomment"), object: nil)
                    }
                    else if self.type == "Public" {
                        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "updatecomment"), object: nil)
                    }
                    else if self.type == "News" {
                        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "updatenewscomment"), object: nil)
                    ...
                    self.arrcomments = [String]()
                    self.arrprofileimage = [String]()
                    self.arrcommentid = [String]()
                    self.getallcomments()
                }
                else {
                ...
                if error == nil && responseObject != nil{
                    if responseObject!.count > 0{
                        ...
                        if datadic.count > 0 {
                            ...
                            self.btnlike.setTitle("\(self.totallike+" liked this")", for: .normal)
...
extension Comments: CZPickerViewDelegate, CZPickerViewDataSource {
